...
const Matches = ({ navigation }) => {
  ...
  const functionGet = async () => {
    ...
    axios.get(`${api_url}/users/my/${myuserId}`).then((res) => {
      console.log(res.data);
  ...
  return (
    <SafeAreaView>
      <View style={{ marginVertical: 10, borderBottomColor: '#ccc', borderBottomWidth: 1 }}>
        <SubHeader2 />
      </View>
      {visible ? <MyLoader top='45%' /> : null}
      <Text style={{ fontSize: 16, fontWeight: 'bold', paddingHorizontal: 20,color:'black' }}>
        Our Matches
      </Text>
      ...
      <ScrollView style={{ width: '100%', height: '87%', padding: 20 }}>
          ...
          return (
            <>
              <View style={styles.card}>
                <ImageBackground style={styles.cardImage} source={{ uri: i?.userFrom?.image1 }} >
                  <View style={{ width: '100%', position: 'absolute', bottom: 0 }}>
                    <LinearGradient colors={[
                      ...
                      <View style={{ padding: 20 }}>
                        <View>
                          <TouchableOpacity pressRetentionOffset={true} style={{ zIndex: 9999 }} onPress={() => navigation.navigate('viewProfile', { 'pairuserid': i?.userFrom?.id })}>

                            <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' }} >

                              <Image source={require('../../components/Images/greenTick.png')} style={{ width: 18, height: 18, resizeMode: 'cover' }} />
                              <Text style={styles.cardTitle}>{i?.userFrom?.firstName + ' ' + i?.userFrom?.lastName}</Text>
                            </View>
                        ...
                        <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 10 }}>
                          <View style={{ flexDirection: 'row' }}>
                            <Text style={{ color: 'white', fontSize: 12 }}>{i?.userFrom?.dob}, {i?.userFrom?.height} </Text>
                            <Text style={{ color: 'white', fontSize: 12, fontWeight: 'bold' }}> . {i?.userFrom?.workingAs} </Text>
                          </View>

                          <View style={{ flexDirection: 'row', gap: 10 }}>

                            {i?.userFrom?.myFollow.includes((myId)) ? (
                            ...
                            <TouchableOpacity onPress={() => submitFollow(i?.userFrom?.id, 'pass')}>
